...
'
    ...
    if is_alpine "${variant}"; then
      ...
      checksum=$(
        curl -sSL --compressed "https://github.com/loong64/node/releases/download/v${nodeVersion}/SHASUMS256.txt" | grep "node-v${nodeVersion}-linux-loong64-musl.tar.xz" | cut -d' ' -f1
      )
      if [ -z "$checksum" ]; then
        rm -f "${dockerfile}-tmp"
